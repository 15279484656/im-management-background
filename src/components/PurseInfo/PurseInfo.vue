  ...
  <div class="content-page">
    ...
    <div class="content-main">
      <div class="table-header">
        <el-button type="primary" @click="handleAddNew">新增钱包记录</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleRowEdit(index, row) {
      this.$router.push({name: 'purse_info_add', query: {id: row.id}});
    },
    handleRowDelete(index, row) {
      this.$confirm('确定要删除这条记录吗？', '提示', {
        ...
        this.axios.delete('/newspace/purseInfo', {params: {id: row.id}}).then(response => {
    ...
    handleAddNew() {
      this.$router.push({name: 'purse_info_add'});
    },
    onSubmitFilter() {
      this.page = 1;
      this.pageSize = 10;
    ...
    getList() {
      ...
      this.axios.get('/newspace/purseInfo/page', {params})
          .then(response => {
            if (response.data.success) {
        ...
        this.$message.error('获取钱包信息失败，请检查网络或联系管理员。');
      });
    }
  },
  mounted() {
